...
3. **Quick scout** — run `node .superpowers/superpowers_status.mjs --repo-root <repo-root> --json` for a fast status snapshot before deeper reads
4. **Read `.superpowers/STATE.md`** — if it exists, understand current chain position
...
Codex agents should read `AGENTS.md` as their primary instruction file (equivalent to Claude Code's `CLAUDE.md`).
...
Treat `.superpowers/state.json` as the machine-readable routing mirror and `.superpowers/STATE.md` as the human-readable narrative. Keep them aligned when updating chain position.
...
node .superpowers/superpowers_status.mjs --repo-root <repo-root> --json  # quick scout
...
1. Re-read `AGENTS.md`
2. Run `node .superpowers/superpowers_status.mjs --repo-root <repo-root> --json`
3. Re-read `.superpowers/HANDOFF.json`, `.superpowers/state.json`, and `.superpowers/STATE.md`
4. Re-read `docs/<feature>/design.md`
5. Re-read the current bead specification
6. Check active file reservations (if in swarming mode)
7. Resume from the phase recorded in STATE.md
...
```
.superpowers/
  onboarding.json     <- onboarding state for the superpowers plugin
...
docs/<feature>/
  ...
  phase-plan.md       <- multi-phase breakdown (multi-phase features only)
  phase-<n>-contract.md  <- current phase entry/exit state
  phase-<n>-story-map.md <- story sequence for current phase
...
Scout scripts live in the superpowers skills directory (not installed per-repo):
- `.superpowers/superpowers_status.mjs` — read-only scout CLI
- `<superpowers-path>/using-superpowers/scripts/superpowers_state.mjs` — shared state helpers
