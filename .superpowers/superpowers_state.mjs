...
/**
 * Superpowers State Management
 *
 * Reads, writes, and normalizes .superpowers/state.json — the machine-readable
 * routing mirror that complements .superpowers/STATE.md.
 *
 * Also provides readSuperpowersStatus() which aggregates onboarding, state,
 * handoff, and critical-patterns into a single status snapshot for the scout.
 *
  ...
  try {
    ...
    while (true) {
      if (
        ...
        fs.existsSync(path.join(candidate, ".superpowers", "onboarding.json"))
      ) {
        return candidate;
      }
...
export function normalizeSuperpowersState(state) {
...
export function getSuperpowersStatePaths(repoRoot) {
  return {
    onboarding: path.join(repoRoot, ".superpowers", "onboarding.json"),
    stateJson: path.join(repoRoot, ".superpowers", "state.json"),
    stateMarkdown: path.join(repoRoot, ".superpowers", "STATE.md"),
    handoff: path.join(repoRoot, ".superpowers", "HANDOFF.json"),
    agents: path.join(repoRoot, "AGENTS.md"),
    criticalPatterns: path.join(repoRoot, "docs", "learnings", "critical-patterns.md"),
  };
}

export function readSuperpowersState(repoRoot) {
  const paths = getSuperpowersStatePaths(repoRoot);
  return normalizeSuperpowersState(readJsonIfExists(paths.stateJson));
}

export function writeSuperpowersState(repoRoot, nextState) {
  const paths = getSuperpowersStatePaths(repoRoot);
  const normalized = normalizeSuperpowersState(nextState);
...
function buildNextReads(status) {
  ...
  if (status.handoff.exists) {
    reads.push(".superpowers/HANDOFF.json");
  }

  if (status.state_json.exists) {
    reads.push(".superpowers/state.json");
  }

  if (status.state_markdown.exists) {
    reads.push(".superpowers/STATE.md");
  }

  const featureSlug = deriveFeatureSlug(status);
  if (featureSlug) {
    reads.push(`docs/${featureSlug}/design.md`);
  }

  if (status.critical_patterns_exists) {
    reads.push("docs/learnings/critical-patterns.md");
  }
...
function buildRecommendedActions(status) {
  if (!status.onboarding.exists) {
    return [
      "Run superpowers onboarding before continuing.",
      "Use the onboarding script in skills/using-superpowers/scripts/.",
    ];
  }
  ...
  if (activeSkill || (phase && phase !== "idle")) {
    return [
      ...
      "Read the active feature design.md before planning or execution work.",
    ];
  }

  return [
    "Use this status snapshot to choose the next skill.",
    "If you move into planning or execution, read critical-patterns.md first when it exists.",
  ];
}

export function readSuperpowersStatus(repoRoot) {
  const paths = getSuperpowersStatePaths(repoRoot);
  ...
  const status = {
    ...
    onboarding: {
      ...
      plugin_version: onboarding?.version || "",
    },
    state_json: {
      exists: Boolean(stateJson),
      ...normalizeSuperpowersState(stateJson),
    },
    state_markdown: {
...
export function renderSuperpowersStatus(status) {
